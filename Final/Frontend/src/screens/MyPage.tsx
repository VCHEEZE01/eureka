...
/**
 * 마이페이지 / 보관함 (PRD F09, F10)
 * 즐겨찾기한 문제를 다시 확인한다. 로그인이 필요하다.
 *
 * ★ 아이디어·개인화 기능은 현재 잠금 상태다. saved/runs 는 타입만 유지되고
 *   실제로는 항상 비어 있으므로 해당 탭·이력 섹션은 자연스럽게 표시되지 않는다.
 */
      ...
      <AppShell>
        <Page>
          <div className="max-w-[560px] flex flex-col gap-6">
            ...
            <Body>
              저장한 문제를 계정에 묶어두기 때문입니다. 문제 탐색은 로그인 없이 계속 보실 수
              있습니다.
            </Body>
      ...
      <Page>
        ...
        {list.length === 0 ? (
          <EmptyState
            ...
            description="문제 상세 화면에서 저장을 누르면 여기에 모입니다."
            action={
        ...
        {/* 개인화 실행 이력 — 현재 잠금 기능이라 runs 는 항상 비어 있다 */}
        {runs.length > 0 ? (
                ...
                <li key={run.id} className="border-t border-border last:border-b">
                  <div className="flex items-center gap-4 py-5">
                    <div className="flex-1 min-w-0">
                      ...
                      <Caption>아이디어 {run.ideas.length}개</Caption>
                    </div>
