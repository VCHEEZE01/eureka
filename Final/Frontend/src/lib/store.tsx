...
/**
 * 프론트엔드 전역 상태.
 * 백엔드 인증이 없으므로 localStorage에 보관한다.
 *
 * 담당 범위: 로그인(F10), 타겟 프로필(온보딩 교체), 문제 선택·조합(F05, 현재 잠금),
 * 개인화 결과(F07, 현재 잠금), 즐겨찾기/보관함(F09).
 *
...
/** ★ 온보딩 계약이 바뀌었으므로(출처 선택 → 타겟 설정) 저장 키를 새로 쓴다. 옛 키는 무시한다. */
const STORAGE_KEY = 'eureka.frontend.v2';
...
interface State {
  user: User | null;
  /** 온보딩에서 입력한 타겟. null이면 아직 온보딩을 안 거쳤다. */
  target: TargetProfile | null;
  /** F05 조합을 위해 문제 탐색에서 체크한 문제들 (현재 잠금 기능) */
...
export function useStore() {
  ...
  const setTarget = useCallback((target: TargetProfile) => set({ target }), []);
