...
| | 1일 | 일주일 | 한달 |
|---|---|---|---|
| AI 행동 | 질문 없이 즉시 전체 코드 | **먼저 질문 → 답 받고 착수** | 아키텍처 설계 → 단계별 구현 |
| IA 반영 범위 | 1Depth 핵심 1~2개만 | 1·2Depth 전체 | 1·2Depth 전체 + `futureFeatures` |
| 데이터 | 인라인 목업 + localStorage | 경량 BaaS | DB 스키마·인증·API 설계 |
| 산출 요구 | 폴더 구조 + 컴포넌트 코드 | Day1~5 로드맵 | 주차별 마일스톤 + ADR |

일주일 프롬프트는 AI가 사용자에게 **먼저 되묻게** 만드는 것이 핵심이다(타깃 사용 맥락 / 우선 화면 / 데이터 출처 / 디자인 톤). 사용자가 답하면서 아이디어가 자기 상황에 맞게 구체화되는 것이 이 기간대의 가치이므로, 이 질문 유도 문구를 지우면 1일 프롬프트와 차별점이 없어진다.
